	...
	<view>
		<view class="search">
			<template v-if="isFocus">
				<input maxlength="20" focus type="text" value="" confirm-type="search" @confirm="searchStart()" placeholder="请输入剧名/演员名称搜索" v-model.trim="searchText"/>
			</template>
			<template v-else>
				<input maxlength="20" type="text" value="" confirm-type="search" @confirm="searchStart()" placeholder="请输入剧名/演员名称搜索" v-model.trim="searchText"/>
			</template>
			<view mode="aspectFit" @click="searchStart()" class="search-icon iconfont icon-soushuo"></view>
		</view>
		<view :class="'s-' + theme" v-if="hList.length > 0">
			<view class="header">
				历史记录
				<view  mode="aspectFit" @click="delhistory" class="deicon iconfont icon-shanchu"></view>
			</view>
			<view class="list">
...
<style lang="less" scoped>
	.search{
		...
		input{
			...
			padding: 10upx 74upx 10upx 20upx;
			font-size: 28upx;
			border-radius: 50upx;
		}
		.voice-icon{
